const Cycles = (() => {
    ...
    function buildCard(c) {
        ...
        if (isOpen) {
            ...
            statusBadge = '<span class="text-xs bg-blue-900/50 text-blue-400 px-1.5 py-0.5 rounded">Ouvert</span>';
            ...
            statusBadge = pnlValue >= 0
                ? '<span class="text-xs bg-green-900/50 text-green-400 px-1.5 py-0.5 rounded">Win</span>'
                : '<span class="text-xs bg-red-900/50 text-red-400 px-1.5 py-0.5 rounded">Loss</span>';
        ...
        <div class="cycle-card ${cardBorder}">
            <div class="flex items-center justify-between mb-1.5">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs font-bold ${sideClass}">${c.side}</span>
                    <span class="text-xs text-gray-500">${marketLabel}</span>
                </div>
                <div class="flex items-center gap-2">
                    <span class="text-xs text-gray-500">${c.duration || ''}</span>
                    ${statusBadge}
                </div>
            </div>
            <div class="grid grid-cols-3 gap-2 text-sm">
                ...
                <div>
                    <div class="text-gray-500 text-xs">${priceLabel}</div>
                    <div>${priceValue}</div>
                </div>
                <div>
                    ...
                    <div class="text-gray-600 text-xs">${pctSign}${pnlPct.toFixed(2)}%</div>
                </div>
            </div>
            <div class="flex items-center justify-between mt-1.5 text-xs text-gray-500">
                <span>$${notional} | fees $${fees.toFixed(2)}</span>
    ...
    async function loadStats(symbol) {
        try {
            ...
            if (tp) {
                ...
                tp.className = `font-semibold ${pnl >= 0 ? 'pnl-positive' : 'pnl-negative'}`;
            }
        } catch (e) {
