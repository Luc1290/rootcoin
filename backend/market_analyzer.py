...
async def _analyze_symbol(symbol: str) -> dict:
    ...
    return {
        ...
        "signals": {
            "technical": [_signal_to_dict(s) for s in ta_signals],
...
def _signal_to_dict(s: dict) -> dict:
    score = s["score"]
    if score > 0.15:
        signal = "bullish"
    elif score < -0.15:
...
def _build_justification(ta_signals: list, macro_signals: list, direction: str) -> str:
    # Group TA signals by timeframe
    tf_groups: dict[str, list] = {}
    for s in ta_signals:
        # Extract timeframe from name e.g. "RSI(1h)" -> "1h"
        name = s["name"]
        tf = "other"
